...
impl ChannelRouter {
    pub fn routes(buses: Buses) -> Router {
        Router::new().nest(
            "/v1/channels",
            Router::new()
                .route("/", get(Self::query_channels))
                .route("/", post(Self::create_channel))
                .nest(
                    "/:channel_id",
                    Router::new()
                        .route("/", get(Self::find_channel))
                        .route("/", put(Self::update_channel))
                        .route("/", delete(Self::delete_channel)),
                )
                .with_state(buses),
        )
    }
    ...
    async fn create_channel(
        ...
        Self::retrieve_channel_resource(&actor, &query_bus, *command.channel_id(), |_| {
